...
import 'package:product_listing_app_test/api_confiq/api_config.dart';
import 'package:product_listing_app_test/model/api_get_products/api_get_products.dart';
import 'package:product_listing_app_test/model/api_get_slider/api_get_slider.dart';
...
Future<List<String>> fetchImageUrls() async {
  try {
    final response = await http.get(Uri.parse(apiUrl.baseUrl+apiUrl.sliderUrl));

    if (response.statusCode == 200) {
      
      List<ApiGetSlider> sliders = apiGetSliderFromJson(response.body);

      // Extract image URLs from the 'image' field in ApiGetSlider
      List<String> imageUrls = sliders.map((slider) => slider.image ?? "").toList();
